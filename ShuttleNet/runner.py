...
def calculateflops(test_dataloader, encoder, decoder, location_MSE_criterion, location_MAE_criterion, shot_type_criterion, args, device="cpu"):
    encode_length = args['encode_length']
    encoder.eval(), decoder.eval()
    ...
    with torch.no_grad():
        for rally, target in tqdm(test_dataloader):
            ...
            total_loss_type += best_type_loss
            total_loss += best_loss
    fencode_local_output, encode_global_A, encode_global_B = encoder(encoder_player, encoder_player_A_x, encoder_player_A_y, encoder_player_B_x, encoder_player_B_y, encoder_shot_type, two_player=player[:, 0:2])
