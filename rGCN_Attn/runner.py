        ...
        for rally, target in tqdm(test_dataloader):
            ...
            for sample_index in range(args['sample_num']):
                ...
                for sequence_index in range(encode_length, length[0]+1):
                    ...
                    plots.append([predict_A_xy,gold_A_xy,predict_B_xy, gold_B_xy,predict_shot_type_logit,decoder_target_type])
            ...
            # best_loc_mse.append(best_location_MSE_loss)
            # best_loc_mae.append(best_location_MAE_loss)
            # best_type.append(best_type_loss)

            if best_location_MSE_loss<0.35:
              pprint.pprint(plots)
              print(len(plots),len(plots[0]))
              with open("plots", "wb") as fp:   #Pickling
                pickle.dump(plots, fp)
              break
