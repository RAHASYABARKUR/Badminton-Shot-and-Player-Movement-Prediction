...
def calculateflops(train_dataloader, valid_dataloader, encoder, decoder, location_criterion, shot_type_criterion, encoder_optimizer, decoder_optimizer, args, device="cpu"):
    ...
    for epoch in tqdm(range(args['epochs'])):
        ...
        #for rally, target in train_dataloader:
            rally,target = next(iter(train_dataloader))
            ...
            encoder_output = encoder(encoder_player, encoder_player_A_x, encoder_player_A_y, encoder_player_B_x, encoder_player_B_y, encoder_shot_type, two_player=player[:, 0:2])
            
            encoder_flops = FlopCountAnalysis(encoder, (encoder_player, encoder_player_A_x, encoder_player_A_y, encoder_player_B_x, encoder_player_B_y, encoder_shot_type, player[:, 0:2]))
            print(f"Encoder FLOPs: {encoder_flops.total()}")
            ...
            predict_xy, predict_shot_type_logit = decoder(decoder_player, decoder_player_A_x, decoder_player_A_y, decoder_player_B_x, decoder_player_B_y, decoder_shot_type, encoder_output, two_player=player[:, 0:2])
            decoder_flops = FlopCountAnalysis(decoder, (decoder_player, decoder_player_A_x, decoder_player_A_y, decoder_player_B_x, decoder_player_B_y, decoder_shot_type, encoder_output, player[:, 0:2]))
            print(f"Decoder FLOPs: {decoder_flops.total()}")
            ...
            # Calculate the number of parameters for Encoder and Decoder
