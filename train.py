...
args.add_argument("--pretrained_model",type=int, default=0)
args.add_argument("--pretrained_model_weights",type=str,default = None)
...
if args['model_type'] == 'MHA_LSTM':
    from MHA_LSTM.model import Encoder, Decoder
    from MHA_LSTM.runner import train
    encoder = Encoder(args)
    decoder = Decoder(args)
    encoder.player_embedding.weight = decoder.player_embedding.weight
    encoder.type_embedding.weight = decoder.type_embedding.weight
...
if args['model_type'] == 'BiLSTM':
    from BiLSTM.model import Encoder, Decoder
    from BiLSTM.runner import train
    encoder = Encoder(args)
    decoder = Decoder(args)
    encoder.player_embedding.weight = decoder.player_embedding.weight
    encoder.type_embedding.weight = decoder.type_embedding.weight
...
if args['model_type'] == 'GRU':
    from GRU.model import Encoder, Decoder
    from GRU.runner import train
    encoder = Encoder(args)
    decoder = Decoder(args)
    encoder.player_embedding.weight = decoder.player_embedding.weight
    encoder.type_embedding.weight = decoder.type_embedding.weight
...
if args['model_type'] == 'LSTM_Attn':
    ...
    from LSTM_Attn.runner import train
    encoder = Encoder(args)
    decoder = Decoder(args)
    encoder.player_embedding.weight = decoder.player_embedding.weight
    encoder.type_embedding.weight = decoder.type_embedding.weight
...
if args['model_type'] == 'rGCN_Attn':
    ...
    from rGCN_Attn.runner import train
    encoder = Encoder(args, device)
    decoder = Decoder(args, device)
    encoder.player_embedding.weight = decoder.player_embedding.weight
    encoder.coordination_transform.weight = decoder.coordination_transform.weight

if args['model_type'] == 'rGCN_Attn_Twice':
    if args['use_complete_graph'] == 1:
        from rGCN_Attn_Twice.model_complete import Encoder, Decoder
    else:
        from rGCN_Attn_Twice.model import Encoder, Decoder
    from rGCN_Attn_Twice.runner import train
    encoder = Encoder(args, device)
    decoder = Decoder(args, device)
    encoder.player_embedding.weight = decoder.player_embedding.weight
    encoder.coordination_transform.weight = decoder.coordination_transform.weight
...
if args['model_type'] == 'rGCN_MHA':
    ...
    from rGCN_MHA.runner import train
    encoder = Encoder(args, device)
    decoder = Decoder(args, device)
    encoder.player_embedding.weight = decoder.player_embedding.weight
    encoder.coordination_transform.weight = decoder.coordination_transform.weight
...
if args['pretrained_model']==1:
    print("Londing Pretrained weights from path provided: ", args['pretrained_model_weights'])
    encoder.load_state_dict(torch.load(args['pretrained_model_weights'] + '/encoder', map_location=torch.device('cpu'))), decoder.load_state_dict(torch.load(args['pretrained_model_weights'] + '/decoder', map_location=torch.device('cpu')))
