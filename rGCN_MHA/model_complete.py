...
class relational_GCN_layer(nn.Module):
    def __init__(self, hidden_size, type_num, num_basis, device):
        ...
        # self.normalization_constant = torch.nn.Parameter(torch.Tensor(1, type_num - 1 + 4))
    ...
    def forward(self, node_embedding, adjacency_matrix, activation_function):
        ...
        # connected_node_embedding = torch.sum(torch.matmul(connected_node_embedding, mutil_relational_weight.unsqueeze(0)) * self.normalization_constant.view(1, self.type_num - 1 + 4, 1, 1), dim=1)
...
class Decoder(nn.Module):
    def __init__(self, args, device):
        ...
        self.model_attention = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=2, batch_first=True)
    ...
    def forward(self, player, step, encode_node_embedding, adjacency_matrix, 
        ...
        if not first:
            ...
            model_input,weights = self.model_attention(model_input,model_input,model_input)
