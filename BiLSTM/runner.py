...
def train(train_dataloader, valid_dataloader, encoder, decoder, location_criterion, shot_type_criterion, encoder_optimizer, decoder_optimizer, args, device="cpu"):
    ...
    for epoch in tqdm(range(args['epochs'])):
        ...
        for rally, target in train_dataloader:
            ...
            hidden = torch.zeros((2, batch_size, args['hidden_size'])).to(device)
            cell = torch.zeros((2, batch_size, args['hidden_size'])).to(device)
            
            hidden, cell = encoder(encoder_player, encoder_player_A_x, encoder_player_A_y, encoder_player_B_x, encoder_player_B_y, encoder_shot_type, hidden, cell, two_player=player[:, 0:2])
            ...
            predict_xy, predict_shot_type_logit, hidden, cell = decoder(decoder_player, decoder_player_A_x, decoder_player_A_y, decoder_player_B_x, decoder_player_B_y, decoder_shot_type, hidden, cell, two_player=player[:, 0:2])
            ...
            #print((predict_A_xy, gold_A_xy),(predict_B_xy, gold_B_xy),"prediction location")
            loss_location = (Gaussian2D_loss(predict_A_xy, gold_A_xy) + Gaussian2D_loss(predict_B_xy, gold_B_xy)) / 2
            #print(loss_location,loss_type, "LOSSESS")
    ...
    with torch.no_grad():
        for rally, target in tqdm(test_dataloader):
            ...
            for sample_index in range(args['sample_num']):
                ...
                hidden = torch.zeros((1, batch_size, args['hidden_size'])).to(device)
                cell = torch.zeros((1, batch_size, args['hidden_size'])).to(device)
                ...
                for sequence_index in range(encode_length, length[0]):
                    ...
                    loss_type = shot_type_criterion(predict_shot_type_logit[:, 0, :], decoder_target_type)
                    if sample_index == 0:
                        total_instance += 1
                    ...
                    decoder_player = player[:, sequence_index:sequence_index+1]
                    ...
                    weights = predict_shot_type_logit[0, 0, 1:]
